...
## Deskripsi Aplikasi
PULSA adalah aplikasi web untuk jurnalisme warga dan partisipasi publik. Pengguna dapat mengirim opini dan suara pembaca; admin dapat memoderasi konten, mengelola tim, dan mengatur pengguna.

Fitur utama:
- Halaman landing untuk berita, opini, dan suara pembaca
- Panel admin untuk moderasi dan pengelolaan konten
- Panel pengguna untuk mengirim dan melihat status opini/suara
...
1. Clone repository ke folder kerja:
...
3. Salin file environment dan atur konfigurasi database:
	...
	Edit `.env` untuk mengatur `DB_DATABASE`, `DB_USERNAME`, `DB_PASSWORD`.

4. Generate application key:

	php artisan key:generate

5. Jalankan migrasi dan seeder (opsional jika tersedia):

	php artisan migrate --seed

6. Buat symlink storage agar file upload dapat diakses:

	php artisan storage:link

7. Install asset front-end dan build:
...
- Jika asset CSS/JS tidak ter-load, jalankan `npm run dev` dan refresh browser.

## Akun Dummy Login
Gunakan akun berikut untuk testing (jika tidak ada, jalankan seeder `UserSeeder` atau buat manual):
